...
LOG_MODULE_REGISTER(eth_xlnx_gem, CONFIG_ETHERNET_LOG_LEVEL);

#if defined(CONFIG_ETH_XLNX_GEM_PORT_0) \
	&& !defined(DT_INST_0_XLNX_GEM)
#error Cannot compile driver for GEM0: device tree configuration data is unavailable!
#endif

#if defined(CONFIG_ETH_XLNX_GEM_PORT_1) \
	&& !defined(DT_INST_1_XLNX_GEM)
#error Cannot compile driver for GEM1: device tree configuration data is unavailable!
#endif
 ...
 * declared below are located in non-cached, non-buffered (strongly ordered) memory!
 */

#if defined(DT_INST_0_XLNX_GEM) \
	&& defined(CONFIG_ETH_XLNX_GEM_PORT_0) \
	&& !defined(CONFIG_ETH_XLNX_GEM_PORT_0_DMA_FIXED)
struct eth_xlnx_dma_area_gem0 dma_area_gem0;
#endif

#if defined(DT_INST_1_XLNX_GEM) \
	&& defined(CONFIG_ETH_XLNX_GEM_PORT_1) \
	&& !defined(CONFIG_ETH_XLNX_GEM_PORT_1_DMA_FIXED)
...
/* Driver instance configuration data for GEM0, if active */

static struct eth_xlnx_gem_dev_cfg eth_xlnx_gem_gem0_dev_cfg = {
	/* Controller base address -> from device tree data */
	.base_addr				= DT_INST_0_XLNX_GEM_BASE_ADDRESS,
	...
	/* AMBA Clock enable bit of the respective GEM in the SLCR  -> from autoconf */
	...
	/* DMA area receive / transmit buffer (descriptor) related data  -> from autoconf */
...
/* Driver instance run-time data for GEM0, if active */

static struct eth_xlnx_gem_dev_data eth_xlnx_gem_gem0_dev_data = {
	#ifdef CONFIG_ETH_XLNX_GEM_PORT_0_MAC_DEVTREE
	/* Obtain the MAC address from the device tree data */
	.mac_addr = DT_XLNX_GEM_ETH0_LOCAL_MAC_ADDRESS,
	#else
...
ETH_NET_DEVICE_INIT(eth_xlnx_gem_gem0, DT_XLNX_GEM_ETH0_LABEL,
		eth_xlnx_gem_dev_init, &eth_xlnx_gem_gem0_dev_data, &eth_xlnx_gem_gem0_dev_cfg,
...
/* Driver instance configuration data for GEM1, if active */

static struct eth_xlnx_gem_dev_cfg eth_xlnx_gem_gem1_dev_cfg = {
	/* Controller base address -> from device tree data */
	.base_addr				= DT_INST_1_XLNX_GEM_BASE_ADDRESS,
...
static struct eth_xlnx_gem_dev_data eth_xlnx_gem_gem1_dev_data = {
	#ifdef CONFIG_ETH_XLNX_GEM_PORT_1_MAC_DEVTREE
	/* Obtain the MAC address from the device tree data */
	.mac_addr = DT_XLNX_GEM_ETH1_LOCAL_MAC_ADDRESS,
	#else
	/* Obtain the MAC address from autoconf */
	.mac_addr = {
...
ETH_NET_DEVICE_INIT(eth_xlnx_gem_gem1, DT_XLNX_GEM_ETH1_LABEL,
		eth_xlnx_gem_dev_init, &eth_xlnx_gem_gem1_dev_data, &eth_xlnx_gem_gem1_dev_cfg,
		CONFIG_ETH_INIT_PRIORITY,
		&eth_xlnx_gem_apis, NET_ETH_MTU);
...
static void eth_xlnx_gem_aux_timer (struct k_timer *timer_id) 
{
	...
	u8_t                            aux_thread_notify = ETH_XLNX_GEM_AUX_THREAD_POLL_PHY_BIT;
...
static void eth_xlnx_gem_aux_thread (void *p1, void *p2, void *p3) 
{
	...
	while (1) {
		...
		if (dev_conf->init_phy == 1 
			&& (aux_thread_notify & ETH_XLNX_GEM_AUX_THREAD_POLL_PHY_BIT) != 0) {
		...
		if ((aux_thread_notify & ETH_XLNX_GEM_AUX_THREAD_RXDONE_BIT) != 0) {
			...
			while (1) {
				...
				if ((reg_val & ETH_XLNX_GEM_RXBD_START_OF_FRAME_BIT) == 0) {
					...
					LOG_DBG("eth_xlnx_gem_aux_thread: unexpected missing SOF bit in RX BD [%u]\n", first_bd_idx);
					break;
				}
				...
				rx_data_length = (u16_t)(reg_val & ETH_XLNX_GEM_RXBD_FRAME_LENGTH_MASK);
				...
				if (dev_data->eff_link_speed != LINK_DOWN) {
					...
					do {
						...
						rx_data_length	= eff_copy_len = (u16_t)(reg_val & ETH_XLNX_GEM_RXBD_FRAME_LENGTH_MASK);
						src_buffer_offs = 0;
					...
					if (net_recv_data(dev_data->iface, pkt) < 0) {
						LOG_DBG("eth_xlnx_gem_aux_thread: packet hand-over to IP stack failed\n");
						net_pkt_unref(pkt);
					}
...
static void eth_xlnx_gem_iface_init (struct net_if *iface) 
{
	...
	/* Initialize the timer for the auxiliary thread if the PHY
	 * of the respective GEM is managed by the corresponding 
	 * driver's instance. */
	...
	#if defined(DT_INST_0_XLNX_GEM) && defined(CONFIG_ETH_XLNX_GEM_PORT_0)
	if (dev_conf->base_addr == DT_INST_0_XLNX_GEM_BASE_ADDRESS) {
		dev_data->aux_thread_tid = k_thread_create(
			&dev_data->aux_thread_data,
			eth_xlnx_gem_aux_thread_stack_gem0,
			K_THREAD_STACK_SIZEOF(eth_xlnx_gem_aux_thread_stack_gem0),
	...
	#if defined(DT_INST_1_XLNX_GEM) && defined(CONFIG_ETH_XLNX_GEM_PORT_1)
	if (dev_conf->base_addr == DT_INST_1_XLNX_GEM_BASE_ADDRESS) {
		dev_data->aux_thread_tid = k_thread_create(
			&dev_data->aux_thread_data,
			eth_xlnx_gem_aux_thread_stack_gem1,
			K_THREAD_STACK_SIZEOF(eth_xlnx_gem_aux_thread_stack_gem1),
			eth_xlnx_gem_aux_thread,
			(void*)dev, (void*)iface, NULL,
			dev_data->aux_thread_prio,
			0,
	...
	dev_conf->config_func(dev); /* effectively: eth_xlnx_gem_irq_config(dev) */
}
...
static void eth_xlnx_gem_irq_config (struct device *dev) 
{
	 ...
	 * Wake-on-LAN IRQs are not supported. */

	#if defined(DT_INST_0_XLNX_GEM) && defined(CONFIG_ETH_XLNX_GEM_PORT_0)
	if (dev_conf->base_addr == DT_INST_0_XLNX_GEM_BASE_ADDRESS) {
		IRQ_CONNECT(DT_INST_0_XLNX_GEM_IRQ_0,
			    DT_INST_0_XLNX_GEM_IRQ_0_PRIORITY,
			    eth_xlnx_gem_isr, DEVICE_GET(eth_xlnx_gem_gem0), 0);
		irq_enable(DT_INST_0_XLNX_GEM_IRQ_0);
	}
	#endif

	#if defined(DT_INST_1_XLNX_GEM) && defined(CONFIG_ETH_XLNX_GEM_PORT_1)
	if (dev_conf->base_addr == DT_INST_1_XLNX_GEM_BASE_ADDRESS) {
		IRQ_CONNECT(DT_INST_1_XLNX_GEM_IRQ_0,
			    DT_INST_1_XLNX_GEM_IRQ_0_PRIORITY,
			    eth_xlnx_gem_isr, DEVICE_GET(eth_xlnx_gem_gem1), 0);
		irq_enable(DT_INST_1_XLNX_GEM_IRQ_0);
	}
	#endif
}
...
static int eth_xlnx_gem_send (struct device *dev, struct net_pkt *pkt) 
{
	...
	bds_reqd = (u8_t)((pkt_len + (dev_conf->tx_buffer_size - 1)) 
	...
	{
		LOG_DBG(
			"eth_xlnx_gem_send: cannot TX, packet length %hu requires %hhu BDs, only have %hhu free\n", 
			pkt_len, 
			bds_reqd, 
	...
	tx_buffer_offs = (u8_t*)(dev_data->first_tx_buffer 
	...
	for (frag = pkt->frags; frag; frag = frag->frags) {
		...
		while (rem_frag_len > 0) {
			if ((used_in_buf + rem_frag_len) <= dev_conf->tx_buffer_size) {
				...
				reg_val  = sys_read32((u32_t)(&dev_data->txbd_ring.first_bd[curr_bd_idx].ctrl));
				reg_val &= ETH_XLNX_GEM_TXBD_WRAP_BIT; /* Preserve the 'wrap' bit */
				reg_val |= ((reg_val & ~ETH_XLNX_GEM_TXBD_LEN_MASK) | dev_conf->tx_buffer_size);
				sys_write32(reg_val, (u32_t)(&dev_data->txbd_ring.first_bd[curr_bd_idx].ctrl));
...
static void eth_xlnx_gem_set_initial_nwcfg (struct device *dev)
{
	...
	if (dev_conf->ignore_igp_rxer == 1)	{
		reg_val |= ETH_XLNX_GEM_NWCFG_IGNIPGRXERR_BIT;                    /* [30]     ignore IPG rx_er */
	}

	if (dev_conf->disable_reject_nsp == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_BADPREAMBEN_BIT;                    /* [29]     disable rejection of non-standard preamble */
	}

	if (dev_conf->enable_igp_stretch == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_IPDSTRETCH_BIT;                     /* [28]     enable IPG stretch */
	}

	if (dev_conf->enable_sgmii_mode == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_SGMIIEN_BIT;                        /* [27]     SGMII mode enable */
	}

	if (dev_conf->disable_reject_fcs_crc_errors == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_FCSIGNORE_BIT;                      /* [26]     disable rejection of FCS/CRC errors */
	}

	if (dev_conf->enable_rx_halfdup_while_tx == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_HDRXEN_BIT;                         /* [25]     RX half duplex while TX enable */
	}

	if (dev_conf->enable_rx_chksum_offload == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_RXCHKSUMEN_BIT;                     /* [24]     enable RX IP/TCP/UDP checksum offload */
	}

	if (dev_conf->disable_pause_copy == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_PAUSECOPYDI_BIT;                    /* [23]     Do not copy pause Frames to memory */
	}

	reg_val |= (((u32_t)(dev_conf->amba_dbus_width) & ETH_XLNX_GEM_NWCFG_DBUSW_MASK)
		 << ETH_XLNX_GEM_NWCFG_DBUSW_SHIFT);                              /* [22..21] Data bus width */

	reg_val |= (((u32_t)mdc_divisor & ETH_XLNX_GEM_NWCFG_MDC_MASK)
		<< ETH_XLNX_GEM_NWCFG_MDC_SHIFT);                                 /* [20..18] MDC clock divisor */

	if (dev_conf->discard_rx_fcs == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_FCSREM_BIT;                         /* [17]     Discard FCS from received frames */
	}

	if (dev_conf->discard_rx_length_errors == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_LENGTHERRDSCRD_BIT;                 /* [16]     RX length error discard */
	}

	reg_val |= (((u32_t)dev_conf->hw_rx_buffer_offset & ETH_XLNX_GEM_NWCFG_RXOFFS_MASK)
		<< ETH_XLNX_GEM_NWCFG_RXOFFS_SHIFT);			                     /* [15..14] RX buffer offset */

	if (dev_conf->enable_pause == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_PAUSEEN_BIT;                        /* [13]     Enable pause TX */
	}

	if (dev_conf->enable_tbi == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_TBIINSTEAD_BIT;                     /* [11]     enable TBI instead of GMII/MII */
	}

	if (dev_conf->ext_addr_match == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_EXTADDRMATCHEN_BIT;                 /* [09]     External address match enable */
	}

	if (dev_conf->enable_1536_frames == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_1536RXEN_BIT;                       /* [08]     Enable 1536 byte frames reception */
	}

	if (dev_conf->enable_ucast_hash == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_UCASTHASHEN_BIT;                    /* [07]     Receive unicast hash frames */
	}

	if (dev_conf->enable_mcast_hash == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_MCASTHASHEN_BIT;                    /* [06]     Receive multicast hash frames */
	}

	if (dev_conf->disable_bcast == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_BCASTDIS_BIT;                       /* [05]     Do not receive broadcast frames */
	}

	if (dev_conf->copy_all_frames == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_COPYALLEN_BIT;                      /* [04]     Copy all frames */
	}

	if (dev_conf->discard_non_vlan == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_NVLANDISC_BIT;                      /* [02]     Receive only VLAN frames */
	}

	if (dev_conf->enable_fdx == 1) {
		reg_val |= ETH_XLNX_GEM_NWCFG_FDEN_BIT;                           /* [01]     enable Full duplex */
	}

	if (dev_conf->max_link_speed == LINK_100MBIT) {
		reg_val |= ETH_XLNX_GEM_NWCFG_100_BIT;                            /* [00]     10 or 100 Mbs */
	} else if (dev_conf->max_link_speed == LINK_1GBIT) {
		reg_val |= ETH_XLNX_GEM_NWCFG_1000_BIT;                           /* [10]     Gigabit mode enable */
	} else {
...
static void eth_xlnx_gem_set_initial_dmacr (struct device *dev)
{
	...
	if (dev_conf->disc_rx_ahb_unavail == 1) {
		reg_val |= ETH_XLNX_GEM_DMACR_DISCNOAHB_BIT;		/* [24]     Discard RX packet when AHB unavailable */
	}

	/* [23..16] DMA RX buffer size in AHB system memory
	 *    e.g.: 0x02 = 128, 0x18 = 1536, 0xA0 = 10240  */

	reg_val |= (((u32_t)dev_conf->ahb_rx_buffer_size & ETH_XLNX_GEM_DMACR_RX_BUF_MASK)
			<< ETH_XLNX_GEM_DMACR_RX_BUF_SHIFT);

	if (dev_conf->enable_tx_chksum_offload == 1) {
		reg_val |= ETH_XLNX_GEM_DMACR_TCP_CHKSUM_BIT;	/* [11]     TX TCP/UDP/IP checksum offload to GEM  */
	}

	if (dev_conf->tx_buffer_size_full == 1) {
		reg_val |= ETH_XLNX_GEM_DMACR_TX_SIZE_BIT;		/* [10]     TX buffer memory size select           */
	}

	/* [09..08] RX packet buffer memory size select
	 *          0 = 1kB, 1 = 2kB, 2 = 4kB, 3 = 8kB */

	reg_val |= (((u32_t)dev_conf->hw_rx_buffer_size << ETH_XLNX_GEM_DMACR_RX_SIZE_SHIFT)
			& ETH_XLNX_GEM_DMACR_RX_SIZE_MASK);

	if (dev_conf->enable_ahb_packet_endian_swap == 1) {
		reg_val |= ETH_XLNX_GEM_DMACR_ENDIAN_BIT;		/* [07]     AHB packet data endian swap enable     */
	}

	if (dev_conf->enable_ahb_md_endian_swap == 1) {
		reg_val |= ETH_XLNX_GEM_DMACR_DESCR_ENDIAN_BIT;	/* [06]     AHB mgmt descriptor endian swap enable */
	}
	...
	reg_val |= ((u32_t)dev_conf->ahb_burst_length & ETH_XLNX_GEM_DMACR_AHB_BURST_LENGTH_MASK);
...
static void eth_xlnx_gem_configure_buffers (struct device *dev)
{
	...
	u32_t							i 			= 0;
	struct eth_xlnx_gem_bd			*bdptr		= NULL; /* Current buffer descriptor pointer */
	...
	if (dev_conf->base_addr == DT_INST_0_XLNX_GEM_BASE_ADDRESS)
	{
			...
			struct eth_xlnx_dma_area_gem0 *dma_area_gem0 = 
				(struct eth_xlnx_dma_area_gem0*)CONFIG_ETH_XLNX_GEM_PORT_0_DMA_BASE_ADDRESS;
	...
	if (dev_conf->base_addr == DT_INST_1_XLNX_GEM_BASE_ADDRESS)
	{
			...
			struct eth_xlnx_dma_area_gem1 *dma_area_gem1 = 
				(struct eth_xlnx_dma_area_gem1*)CONFIG_ETH_XLNX_GEM_PORT_1_DMA_BASE_ADDRESS;
	...
	/* The BD ring data other than the base RX/TX buffer pointers will be set in eth_xlnx_gem_iface_init(). */
	...
	bdptr->addr = (u32_t)((u32_t)dev_data->first_rx_buffer 
		+ (i * (u32_t)dev_conf->rx_buffer_size)) | ETH_XLNX_GEM_RXBD_WRAP_BIT;

	/* Set initial TX BD data -> comp. Zynq-7000 TRM, Chapter 16.3.5, "Transmit Buffer Descriptor List" */
	/* TX BD ring data has already been set up in eth_xlnx_gem_iface_init(). */
...
static int eth_xlnx_gem_mdio_read (u32_t base_addr, u8_t phy_addr, u8_t reg_addr)
{
	...
	reg_val  = ETH_XLNX_GEM_PHY_MAINT_CONST_BITS; /* Set the bits constant for any operation */
	reg_val |= ETH_XLNX_GEM_PHY_MAINT_READ_OP_BIT; /* Indicate a read operation */
	reg_val |= (((u32_t)phy_addr & ETH_XLNX_GEM_PHY_MAINT_PHY_ADDRESS_MASK) 
		<< ETH_XLNX_GEM_PHY_MAINT_PHY_ADDRESS_SHIFT); /* PHY address */
	reg_val |= (((u32_t)reg_addr & ETH_XLNX_GEM_PHY_MAINT_REGISTER_ID_MASK) 
		<< ETH_XLNX_GEM_PHY_MAINT_REGISTER_ID_SHIFT); /* Register address */
...
static void eth_xlnx_gem_mdio_write (u32_t base_addr, u8_t phy_addr, u8_t reg_addr, u16_t value)
{
	...
	reg_val  = ETH_XLNX_GEM_PHY_MAINT_CONST_BITS; /* Set the bits constant for any operation */
	reg_val |= ETH_XLNX_GEM_PHY_MAINT_WRITE_OP_BIT; /* Indicate a read operation */
	reg_val |= (((u32_t)phy_addr & ETH_XLNX_GEM_PHY_MAINT_PHY_ADDRESS_MASK) 
		<< ETH_XLNX_GEM_PHY_MAINT_PHY_ADDRESS_SHIFT); /* PHY address */
	reg_val |= (((u32_t)reg_addr & ETH_XLNX_GEM_PHY_MAINT_REGISTER_ID_MASK) 
		<< ETH_XLNX_GEM_PHY_MAINT_REGISTER_ID_SHIFT);  /* Register address */
	reg_val |= ((u32_t)value & ETH_XLNX_GEM_PHY_MAINT_DATA_MASK); /* 16 bits of data for the destination register */
...
static void eth_xlnx_gem_phy_detect (struct device *dev)
{
	...
	for (phy_addr = 1; phy_addr <= 32; phy_addr++) {
		phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, phy_addr, PHY_IDENTIFIER_1_REGISTER);
		phy_id    = (((u32_t)phy_data << 16) & 0xFFFF0000);
		phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, phy_addr, PHY_IDENTIFIER_2_REGISTER);
		phy_id   |= ((u32_t)phy_data & 0x0000FFFF);

		if (phy_id != 0x00000000 && phy_id != 0xFFFFFFFF) {
			dev_data->phy_addr = phy_addr;
			dev_data->phy_id   = phy_id;
			LOG_DBG("eth_xlnx_gem_phy_detect: PHY detected at address %hhu: ID 0x%08X", phy_addr, phy_id);
...
static void eth_xlnx_gem_phy_reset (struct device *dev)
{
	...
	phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_CONTROL_REGISTER);
	phy_data |= 0x8000;  /* Reset bit */
	phy_data &= ~0x1000; /* Auto-neg disable (for now) */
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_CONTROL_REGISTER, phy_data);
...
static void eth_xlnx_gem_phy_configure (struct device *dev)
{
	...
	phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_PAGE_SWITCH_REGISTER);
	phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_GENERAL_CONTROL_1_REGISTER);

	/* [2..0] 000 = RGMII (System Mode) to Copper */

	phy_data &= ~(PHY_MODE_CONFIG_MASK << PHY_MODE_CONFIG_SHIFT);	
	phy_data |= 0;
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_GENERAL_CONTROL_1_REGISTER, phy_data);
	...
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_PAGE_SWITCH_REGISTER, PHY_BASE_REGISTERS_PAGE);
	...
	phy_data  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_CONTROL_1_REGISTER);
	phy_data &= ~(PHY_MDIX_CONFIG_MASK << PHY_MDIX_CONFIG_SHIFT);
	phy_data |= (0x03 << PHY_MDIX_CONFIG_SHIFT); 
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_CONTROL_1_REGISTER, phy_data);
	...
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_INTERRUPT_ENABLE_REGISTER, phy_data);
	...
	phy_data = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_INTERRUPT_STATUS_REGISTER);
	...
	phy_data_gbit  = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_1000BASET_CONTROL_REGISTER);
	...
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_1000BASET_CONTROL_REGISTER, phy_data_gbit);
	eth_xlnx_gem_mdio_write(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_AUTONEG_ADV_REGISTER, phy_data);
...
static u8_t eth_xlnx_gem_phy_poll_link_status (struct device *dev) 
{
	...
	phy_data = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_STATUS_REGISTER);
...
static enum eth_xlnx_link_speed eth_xlnx_gem_phy_poll_link_speed (struct device *dev) 
{
	...
	phy_data   = eth_xlnx_gem_mdio_read(dev_conf->base_addr, dev_data->phy_addr, PHY_COPPER_STATUS_1_REGISTER);
